...
# 1. Valeur Pollinique ----------------------
yvoz <- read.csv(here::here(traitfolder, "proxys stephane cordeau", "Valeurs polliniques estimees.csv"), sep=";")
dim(yvoz)
# clean species name
yvoz$taxa <- clean_taxo(yvoz$latin_name)
# replace if known synonyms (none ...)
is_syn <- yvoz$taxa %in% synonyms$synonym_taxa
yvoz$taxa[is_syn] <- synonyms$accepted_taxa[match(yvoz$taxa[is_syn], synonyms$synonym_taxa)]
...
newlab <- paste(c("original_taxa", meta$new.name[meta$database%in%"Yvoz_ValPol"]), "Yvoz_ValPol", sep="_")
...
vignes$taxa[is_syn] <- synonyms$accepted_taxa[match(vignes$taxa[is_syn], synonyms$synonym_taxa)]
...
newlab <- paste(c("original_taxa", meta$new.name[meta$database%in%"SPVignes"]), "SPVignes", sep="_")
...
vinedivers <- readxl::read_xlsx(here::here(spfolder, "At_Fr_Ro_Sp_Species_List_VINEDIVERS.xlsx"), sheet=2)
...
vinedivers$taxa[is_syn] <- synonyms$accepted_taxa[match(vinedivers$taxa[is_syn], synonyms$synonym_taxa)]
...
newlab <- paste(c("original_taxa", meta$new.name[meta$database%in%"At_Vinedivers"]), "At_Vinedivers", sep="_")
...
write.csv(out, 
  file = here::here("data", "derived-data", "traitC_privatedb.csv"), row.names=FALSE)
# missing traits
