...
keepTR <- !is.na(taxolist$accepted_taxref) & taxolist$taxref_rank %in%c("ES", "SSES", "VAR")
keepGB <- !is.na(taxolist$accepted_gbif) & taxolist$gbif_rank %in%c("SPECIES", "SUBSPECIES", "VARIETY")
...
flowering_date <- bien_db$trait_name %in% "plant flowering begin" & bien_db$unit %in% "date"
# subset the month from the date
bien_db$trait_value[flowering_date] <- substr(bien_db$trait_value[flowering_date], 1, regexpr("/", bien_db$trait_value[flowering_date])-1)
# update the unit
...
trait_av <- tapply(as.numeric(bien_num$trait_value), list(bien_num$scrubbed_species_binomial, bien_num$trait_lab), mean, na.rm=TRUE)
...
trait_cat <- tapply(bien_cat$trait_value, list(bien_cat$scrubbed_species_binomial, bien_cat$trait_lab), concat)
...
out <- cbind(
  ...
  "original_taxa"=ifelse(is.na(m_num)&is.na(m_cat), NA, taxolist$accepted_taxa),
  trait_av[m_num,], trait_cat[m_cat,]
)

write.csv(out, 
  file = here::here("data", "derived-data", "traitD_BIEN.csv"), row.names=FALSE)
# missing traits
