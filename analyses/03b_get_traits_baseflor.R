...
baseflor <- readxl::read_xlsx(here::here(
  "data",
  "raw-data",
  "traits",
  "baseflor.xlsx"
...
baseflor$taxa <- gsub(
  " \\*$",
  "",
  clean_species_list(baseflor$nomH, iter = TRUE)
)
# replace if known synonyms
is_syn <- baseflor$taxa %in% synonyms$synonym_taxa
baseflor$taxa[is_syn] <- synonyms$accepted_taxa[match(
  baseflor$taxa[is_syn],
...
print(prop.table(table(!is.na(m_baseflor)))) #79%

# 2. Format trait data ---------------------------

keepT <- meta$original.name[meta$database %in% "Baseflor"]
newlab <- paste(
  c("original_taxa", meta$new.name[meta$database %in% "Baseflor"]),
  "Baseflor",
  sep = "_"
)
