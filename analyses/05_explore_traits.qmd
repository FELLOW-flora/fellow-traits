---
title: "Exploration of FELLOW trait dataset"
execute:
  echo: false
format: pdf
---


The objective of this document is to:  

- visually explore the trait database
- understand trait coverage / data gaps
- check for possible inconsistencies  


```{r load}
#| warning: false
#| echo: false
devtools::load_all()

# Load species list with taxonomy
traits <- read.csv(
  here::here("data", "derived-data", "merged_traits.csv")
)

# Load species list with taxonomy
taxolist <- read.csv(
  here::here("data", "derived-data", "species_list_taxo.csv")
)
# taxolist$original_taxa[is.na(taxolist$accepted_taxa)]
# remove NAs
taxolist <- taxolist[!is.na(taxolist$accepted_taxa), ]

# Load short species list
shortlist <- read.csv(
  here::here("data", "derived-data", "species_short_list.csv")
)
```


## Description of the species list

We compiled the species lists from `{r} sum(grepl("^in_", names(taxolist)))`  datasets. 
After cleaning and harmonization, there were `{r} nrow(shortlist)` unique taxa.

```{r}
table(shortlist$wfo_rank)
```

{{< pagebreak >}}


```{r}
# get the database data.frame
col_db <- grep("^in_", names(taxolist))
db <- taxolist[, col_db]
# merge per species (remove duplicates)
dbpersp <- rowsum(db, group = taxolist$accepted_taxa)
# select taxa in short_df (with complete taxonomic rank)
dbpersp <- dbpersp[row.names(dbpersp) %in% shortlist$accepted_taxa, ]
# transform as binary
dbpersp[dbpersp > 1] <- 1
# count the number of database per species
nsp_db <- apply(dbpersp, 2, sum)
# count the number of species per database
ndb <- apply(dbpersp, 1, sum)

# split species between rare and common
th_gen <- floor(length(col_db) / 2) # or max(ndb) / 2
```


Let's define:  
*rare*: a taxa that occurred only in a singe database  
*common*: a taxa that is listed in 50% of the databases (`{r} th_gen+1` out of `{r} length(col_db)`)  

```{r}
#| fig-height: 6
#| fig-width: 8

sp_class <- cut(
  ndb,
  breaks = c(0, 1, th_gen, max(ndb)),
  labels = c("rare", "other", "common")
)

table(sp_class)
sum_gen <- as.matrix(rowsum(dbpersp, sp_class))
colnames(sum_gen) <- gsub("^in_", "", colnames(sum_gen))
par(mar = c(4, 10, 1, 1))
barplot(
  sum_gen,
  horiz = TRUE,
  las = 1,
  xlab = "Number of species",
  col = RColorBrewer::brewer.pal(3, "Set3"),
  cex.names = 0.8
)
legend(
  "topright",
  fill = RColorBrewer::brewer.pal(3, "Set3"),
  legend = c("rare", "other", "common")
)
```

{{< pagebreak >}}

## Description of trait databases

```{r}
real_traits <- !grepl("original", names(traits)) &
  names(traits) != "accepted_taxa"

# summary of NAs
num_na <- apply(is.na(traits), 2, sum)

db_na <- num_na[grep("original", names(num_na))]
names(db_na) <- gsub("original_taxa_", "", names(db_na))

trait_na <- apply(is.na(traits[, real_traits]), 2, sum)

# species with lowest completness
sp_na <- apply(is.na(traits[, real_traits]), 1, sum)
```


So far, we compiled `{r} sum(real_traits)` traits for `{r} nrow(traits)` taxa gathered from `{r} length(db_na)` trait databases. But there are many missing values.

```{r nabase}
# database with best match
par(mar = c(4, 8, 1, 1), cex = 0.9)
barplot(
  sort(db_na),
  horiz = TRUE,
  las = 1,
  xlab = "Number of missing species"
)
```



```{r natraits}
# summary of NAs
trait_db <- strsplit(names(trait_na), "_") |>
  sapply(function(x) x[[length(x)]]) |>
  factor(
    levels = names(db_na)[order(db_na)],
    ordered = TRUE
  )
# database with best match
par(mar = c(4, 8, 1, 1), cex = 0.9)
boxplot(
  trait_na ~ trait_db,
  horizontal = TRUE,
  las = 1,
  xlab = "Number of missing traits values",
  ylab = ""
)
```



```{r naspecies}
#| fig-height: 4
# hist(sp_na, xlab = "Number of missing traits", ylab = "Number of species")

# barplot(t(table(sp_na, taxolist$gbif_rank)), border = NA)
bks <- seq(0, ceiling(max(sp_na) / 10) * 10, by = 10)
df <- data.frame(
  "cat" = cut(sp_na, breaks = bks, include.lowest = TRUE),
  "rank" = shortlist$wfo_rank
)
tdf <- table(df$rank, df$cat)
par(mar = c(4, 4, 1, 1))
barx <- barplot(
  tdf,
  col = hcl.colors(nrow(tdf), "viridis"),
  names = rep("", ncol(tdf)),
  xlab = "Number of missing traits",
  ylab = "Number of species"
)
barax <- c(barx - mean(diff(barx) / 2), max(barx) + mean(diff(barx) / 2))
axis(1, at = barax, bks)
legend(
  "topleft",
  legend = firstup(row.names(tdf)),
  fill = hcl.colors(nrow(tdf), "viridis")
)
```


```{r}
#| fig-height: 3.8
rm_dup <- !duplicated(traits$accepted_taxa)
# table(traits$accepted_taxa[rm_dup] == names(ndb))

par(mar = c(4, 4, 1, 1))
plot(
  jitter(ndb),
  sp_na[rm_dup],
  xlab = "Number of datasets (jittered)",
  ylab = "Number of missing traits"
)
abline(v = c(1.5, th_gen + 0.5), lwd = 2)
segments(
  c(par()$usr[1], 1.5, th_gen + 0.5),
  par()$usr[3],
  c(1.5, th_gen + 0.5, par()$usr[2]),
  par()$usr[3],
  col = RColorBrewer::brewer.pal(3, "Set3"),
  lwd = 5,
  xpd = NA
)
mtext(
  at = c(0.5, th_gen * 1.5),
  side = 1,
  line = 0.5,
  text = c("rare", "common"),
  xpd = NA
)
```


{{< pagebreak >}}


Taxa with no or limited trait information (N=`{r} sum(sp_na >=max(sp_na))`).
```{r nodata}
nodata <- traits$accepted_taxa[sp_na >= max(sp_na)]
print(nodata)
```

*Open question:*  

**How to deal with families taxa?**  

**How to deal with missing trait values?** 

{{< pagebreak >}}

**Summary of trait completness**  
```{r traitcat}
trname <- names(traits)[real_traits]

trone <- sapply(strsplit(trname, "_"), function(x) x[[1]])
trone <- gsub(".mm2.mg.1", "", trone) # SLA must be checked in metatraits
trone <- gsub("Fruit.colour", "Fruit.color", trone) # to be replaced in metatraits
trone[grepl("Flower.UV.reflectance", trone)] <- "Flower.UV.reflectance"

# calculate the completeness of each traits
trna <- apply(!is.na(traits[, real_traits]), 1, as.numeric)
summaryna <- apply(rowsum(trna, trone) > 0, 1, sum)
# to be removed: variables with no information
summaryna <- summaryna[summaryna > 0]

completness <- data.frame(
  "trait" = names(summaryna),
  "database" = as.numeric(table(trone[trone %in% names(summaryna)])),
  "N_taxa" = as.numeric(summaryna),
  "completness" = round(as.numeric(summaryna) / nrow(traits) * 100)
)

# DT::datatable(completness)
knitr::kable(
  completness[order(completness$N_taxa, decreasing = TRUE), ],
  row.names = FALSE,
  col.names = c("Trait", "N database", "N taxa", "Completness (%)")
)
```


{{< pagebreak >}}

## Comparison and merge

### SLA

There are four sources of information for Specific leaf area (SLA) : Hodgson et al. 2023 (in mm2/mg), Ladouceur et al. 2019 (in cm2/g), GIFT (in cm2/g), and BIEN (in m2/kg = mm2/mg).

To merge and fill the SLA values (in m2/kg), we used by order of preference:  

- Hodgson et al. 2023
- Ladouceur et al. 2019
- GIFT
- BIEN


```{r sla}
# names(traits)[grep("SLA", names(traits))]
sla <- data.frame(
  "Hodgson" = traits$SLA_mm2.mg.1_Hodgson2023,
  "Ladouceur" = traits$SLA_cm2.g.1_Ladouceur2019 / 10,
  "GIFT" = traits$SLA_cm2.g.1_GIFT / 10 #,
  #"BIEN" = traits$SLA_m2.kg.1_BIEN
)

sla$filled <- apply(sla, 1, function(x) x[!is.na(x)][1])

pairs(sla, lower.panel = panel.smooth, upper.panel = panel.cor)
```

**Number of NAs:**  
```{r fillsla}
print(apply(is.na(sla), 2, sum))
```


Values are highly correlated, but values from *Ladouceur et al. 2023* are only weakly correlated to other data sources (r<0.65) maybe due to different measuring protocol (?).


{{< pagebreak >}}


### Plant height

There are seven sources of information for plant height. SPVignes, FlorealData, and Ladouceur_2019 are limited to small plants (<1m) (no trees) *and FlorealData>100cm must be clean*.  

To merge and fill the plant height values (in m), we used by order of preference:  

- Lososova et al. 2023
- Ladouceur et al. 2019
- GIFT
- BIEN
- FlorealData
- Ecoflora
- SpVignes


```{r height}
#| fig-height: 4

# names(traits)[grep("height", names(traits))]

# transform height in FlorealData
traits$Plant.height_cm_FlorealData <- ifelse(
  traits$Plant.height_cm_FlorealData >= 100,
  NA,
  traits$Plant.height_cm_FlorealData
)
height <- data.frame(
  "Lososova" = traits$Plant.height_m_Lososova2023,
  "Ladouceur" = traits$Plant.height_cm_Ladouceur2019 / 100,
  "GIFT" = traits$Plant.height_mean_m_GIFT,
  #"BIEN" = traits$Plant.height_m_BIEN,
  "FlorealData" = traits$Plant.height_cm_FlorealData / 100,
  "Ecoflora" = traits$Plant.height_m_Ecoflora,
  "SpVignes" = traits$Plant.height_cm_SPVignes / 100
)

height$filled <- apply(height, 1, function(x) x[!is.na(x)][1])

pairs(height, lower.panel = panel.smooth, upper.panel = panel.cor)
```



**Number of NAs:**  
```{r fillheight}
print(apply(is.na(height), 2, sum))
```

{{< pagebreak >}}

### Seed mass
There are six sources of information for seed mass. To merge and fill the seed mass plant height values (in mg), we used by order of preference:  

- Lososova et al. 2023
- Ladouceur et al. 2019
- GIFT
- BIEN
- Ecoflora
- Biolflor

```{r seed}
#| fig-height: 5
# names(traits)[grep("Seed", names(traits))]

seed <- data.frame(
  "Lososova" = traits$Seed.mass_mg_Lososova2023,
  "Ladouceur" = traits$Seed.mass_mg_Ladouceur2019,
  "GIFT" = traits$Seed.mass_g_GIFT * 1000,
  #"BIEN" = traits$Seed.mass_mg_BIEN,
  "Ecoflora" = traits$Seed.mass_mg_Ecoflora,
  "Biolflor" = traits$Seed.mass_mg_Biolflor
)

seed$filled <- apply(seed, 1, function(x) x[!is.na(x)][1])


pairs(seed, lower.panel = panel.smooth, upper.panel = panel.cor, log = "xy") |>
  suppressWarnings()

```

**Number of NAs:**  
```{r fillseed}
print(apply(is.na(seed), 2, sum))
```



{{< pagebreak >}}

### Flower colour
There are five sources of information for flower colour, but it must be cleaned before colors can be merged.

```{r color}
# names(traits)[grep("col", names(traits))]
color <- data.frame(
  "Baseflor" = traits$Flower.color_Baseflor,
  #"BIEN" = traits$Flower.color_BIEN,
  "GIFT" = traits$Flower.color_GIFT,
  "FlorealData" = traits$Flower.color_FlorealData,
  "YvozValPol" = traits$Flower.color_YvozValPol
)
print(apply(is.na(color), 2, sum))

# removing unclear color patterns for genus
# bien <- color$BIEN
# bien[grepl("_", bien)] <- NA
# table(bien)

# removing unclear color patterns for genus
FlorealData <- color$FlorealData
FlorealData[grepl("_", FlorealData)] <- NA
# table(FlorealData)

# table(bien, FlorealData)

# removing unclear color patterns for genus
baseflor <- color$Baseflor
baseflor[grepl("_", baseflor)] <- NA
table(baseflor)
```


{{< pagebreak >}}

### Dispersal mode
There are three sources of information for dispersal mode, but it must be harmonized before they can be merged.

```{r dispersal}
# names(traits)[grep("Dispersal", names(traits))]
dispersal <- data.frame(
  "Lososova2023" = traits$Dispersal.mode_Lososova2023,
  "GIFT_1" = traits$Dispersal.mode_1_GIFT,
  "GIFT_2" = traits$Dispersal.mode_2_GIFT,
  "Baseflor" = traits$Dispersal.mode_Baseflor
)
print(apply(is.na(dispersal), 2, sum))

dispersal$Lososova2023[grepl("_", dispersal$Lososova2023)] <- NA
dispersal$Baseflor[grepl("_", dispersal$Baseflor)] <- NA
dispersal$GIFT_1[grepl("_", dispersal$GIFT_1)] <- NA
dispersal$GIFT_2[grepl("_", dispersal$GIFT_2)] <- NA

# table("Lososova2023" = dispersal$Lososova2023, "Baseflor" = dispersal$Baseflor)
# table("Lososova2023" = dispersal$Lososova2023, "GIFT" = dispersal$GIFT_2)
# table("Baseflor" = dispersal$Baseflor, "GIFT" = dispersal$GIFT_2)
table("Lososova2023" = dispersal$Lososova2023)
table("GIFT" = dispersal$GIFT_2)
table("Baseflor" = dispersal$Baseflor)
```


{{< pagebreak >}}

### Inflorescence

There are two sources of information for inflorescence. GIFT database considers 11 categories, while Baseflor considers 39 categories. So the categories must be harmonized before merging.

```{r inflore}
#  names(traits)[grep("Inflorescence", names(traits))]
inflore <- data.frame(
  "GIFT" = traits$Inflorescence_GIFT,
  "Baseflor" = traits$Inflorescence_Baseflor
)
print(apply(is.na(inflore), 2, sum))

inflore$GIFT[grepl("_", inflore$GIFT)] <- NA
inflore$Baseflor[grepl("_", inflore$Baseflor)] <- NA

# table("GIFT" = inflore$GIFT, "Baseflor" = inflore$Baseflor, useNA = "ifany")
table("GIFT" = inflore$GIFT)
table("Baseflor" = inflore$Baseflor)
```


{{< pagebreak >}}

### Lifecycle

There are two sources of information for lifecycle. The categories are quite coherent among GIFT and Hodgson2023 if `Herb.peren` and `Woody.peren` are merged into `Perennial`.

```{r lifecycle}
# names(traits)[grep("Lifecycle", names(traits))]
lifecycle <- data.frame(
  "Hodgson2023" = traits$Lifecycle_Hodgson2023,
  "GIFT" = traits$Lifecycle_GIFT
)
print(apply(is.na(lifecycle), 2, sum))

# simplify genus categories
lifecycle$GIFT[grepl("_", lifecycle$GIFT)] <- NA
lifecycle$Hodgson2023[grepl("_", lifecycle$Hodgson2023)] <- NA

table(
  "Hodgson2023" = lifecycle$Hodgson2023,
  "GIFT" = lifecycle$GIFT,
  useNA = "ifany"
)
```


### Clonal index

There are two sources of information for clonal index (Ladouceur 2019 and CLOPLA) but most of them are incomplete. The information seems coherent among the two databases. The order of preference is Ladouceur 2019 then CLOPLA.

```{r clonal}
# names(traits)[grep("Clonal", names(traits))]
clonal <- data.frame(
  "Ladouceur2019" = traits$Clonal.index_Ladouceur2019,
  "CLOPLA" = traits$Clonal.index_CLOPLA
)
clonal$filled <- apply(clonal, 1, function(x) x[!is.na(x)][1])
print(apply(is.na(clonal), 2, sum))

# simplify genus categories
clonal$Ladouceur2019[grepl("_", clonal$Ladouceur2019)] <- NA
clonal$CLOPLA[grepl("_", clonal$CLOPLA)] <- NA

table(
  "Ladouceur2019" = clonal$Ladouceur2019,
  "CLOPLA" = clonal$CLOPLA,
  useNA = "ifany"
)

# table("filled" = clonal$filled)
```