...
midolo <- readxl::read_xlsx(here::here(traitfolder, "disturbance_indicator_values_Midolo_2023.xlsx"))
# clean species name
midolo$taxa <- clean_taxo(midolo$species)
# replace if known synonyms
is_syn <- midolo$taxa %in% synonyms$synonym_taxa
midolo$taxa[is_syn] <- synonyms$accepted_taxa[match(midolo$taxa[is_syn], synonyms$synonym_taxa)]
...
newlab <- paste(c("original_taxa", meta$new.name[meta$database%in%"Midolo_2023"]), "Midolo_2023", sep="_")
...
tichy <- readxl::read_xlsx(here::here(traitfolder, "Ellenberg_Indicator_values_Tichy_2022.xlsx"), sheet = 10)
...
tichy$taxa[is_syn] <- synonyms$accepted_taxa[match(tichy$taxa[is_syn], synonyms$synonym_taxa)]
...
newlab <- paste(c("original_taxa", meta$new.name[meta$database%in%"Tichy_2022"]), "Tichy_2022", sep="_")
...
lososova <- readxl::read_xlsx(here::here(traitfolder, "Lososova_et_al_2023_Dispersal_version2_2024-06-14.xlsx"))
# clean and check synonyms
lososova$taxa <- clean_taxo(lososova$Taxon)
is_syn <- lososova$taxa %in% synonyms$synonym_taxa
lososova$taxa[is_syn] <- synonyms$accepted_taxa[match(lososova$taxa[is_syn], synonyms$synonym_taxa)]
...
newlab <- paste(c("original_taxa", meta$new.name[meta$database%in%"Lososova_2023"]), "Lososova_2023", sep="_")
...
dfevojan$taxa[is_syn] <- synonyms$accepted_taxa[match(dfevojan$taxa[is_syn], synonyms$synonym_taxa)]
...
keepT <- c("FloraVeg.Taxon", meta$original.name[meta$database%in%"Dfevojan_2023"])
newlab <- paste(c("original_taxa", meta$new.name[meta$database%in%"Dfevojan_2023"]), "Dfevojan_2023", sep="_")
...
write.csv(out, 
  file = here::here("data", "derived-data", "traitA_floraveg.csv"), row.names=FALSE)
# missing traits
